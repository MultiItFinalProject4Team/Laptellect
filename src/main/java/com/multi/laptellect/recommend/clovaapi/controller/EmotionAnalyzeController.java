...
public class EmotionAnalyzeController {
    ...
    public void onApplicationEvent() {
        ...
        // 상품 번호별로 리뷰 그룹화
        Map<Integer, List<ReviewDTO>> reviewsByProduct = new HashMap<>();
        for (ReviewDTO review : reviews) {
            int productNo = review.getProductNo();
            if (!reviewsByProduct.containsKey(productNo)) {
                reviewsByProduct.put(productNo, new ArrayList<>());
            }
            reviewsByProduct.get(productNo).add(review);
        }
