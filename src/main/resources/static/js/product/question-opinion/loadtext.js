    ...
    <div class="reply">
        <div class="reply-header">
            <div class="reply-author">
                ...
                <a class="reply-update" onclick="updateQuestion(${productqList.productqNo})">${(productqList.memberNo == memberNo) ? '수정':''}</a>
                <a class="reply-delete" onclick="confirmDelete(${productqList.productqNo})">${(productqList.memberNo == memberNo) ? '/삭제':''}</a>
            </div>
        </div>
        <div class="reply-body">
            <div class="reply-bodytop">
                <h4 class="reply-title">
                    ${(productqList.secret === 'Y' && productqList.memberNo != memberNo) ? '비밀글입니다 🔒' : productaList.title}
                </h4>
            </div>
            <div class="reply-content">
                <p class="reply-content">
                    ${(productqList.secret === 'Y' && productqList.memberNo != memberNo) ? '본인만 확인 가능합니다' : productaList.content}
                </p>
            </div>
        </div>
        <hr class="question-hr">
